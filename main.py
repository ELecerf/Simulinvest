...
def run_simulation(params: dict):
    ...
    # Le taux de crédit est déjà passé en décimal (0.03 = 3 %)
    t_mens = p["taux_credit"] / 12
    ...
    for m in range(1, n_months + 1):
        ...
        # Patrimoine net instantané = valeur bien – dette restante
    ...
    for m in range(1, n_months + 1):
        diff = effort_ref - loyer_P  # montant disponible pour investir
    ...
    patr_final_C = valeur_finale_L + port_C  # dette remboursée
...
if st.sidebar.button("Lancer la simulation"):
    params = dict(
        ...
        apport_pct=10,
        frais_not_pct=8,
        taux_credit=0.03,  # 3 % => 0.03 en décimal
        horizon=horizon,
